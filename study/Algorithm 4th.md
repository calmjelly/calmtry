...
希尔排序其实也是插入排序，但是他不像插入排序那样一步一步的比较移动插入，只能交换相邻的元素，而是跳跃式的比较移动。
通过某个增量将数组划分为若干组，分别对每组进行插入排序，
然后逐步缩小增量，再对缩小增量后的分组进行排序，
最终增量为1，对基本有序的数组进行插入排序。

```java
 for (int h = arr.length / 2; h >= 1; h = h / 2) {
...
不需要额外的内存空间。时间复杂度低于O(n2)。

希尔排序在执行过程中，进行了多次插入排序，单次插入排序是稳定的，但在不同的插入排序过程中，相同的元素可能在各自的插入排序中移动，最后其稳定性就会被打乱，所以希尔排序是不稳定排序。

## 归并排序

先将数组(递归的)分成两半分别排序，然后将结果合并起来。
