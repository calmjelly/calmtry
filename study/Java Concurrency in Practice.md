...
虽然平时写代码用到的不多……但是并不妨碍这一块的重要性。

主要以《Java 并发编程实践》 为主，遇到不熟悉的东西谷狗补充。
...
在每次修改时候，都会加锁并创建一个容器副本，从而实现读写分离。当然，这种情况下，读取的可能不是最新值。

优点：读不加锁，读多写少情况下性能很高。

缺点：写操作复制了一个容器副本，内存占用较多。无法保证数据的强一致性，写操作在另外的副本上，读操作可能读取的不是最新值。



CopyOnWriteArraySet

它底层使用了CopyOnWriteArrayList，add操作会遍历整个list，如果没有相同元素才会放入，remove、set操作同理，都会遍历数组，复杂度O(n)。
...
可定时、可轮询、可中断的锁获取操作使用显式锁，其他非必要情况建议使用synchronized。

synchronized被优化了很多，并不在是一个重量级锁了。

synchronized的锁膨胀过程：偏向锁->轻量级锁->自旋锁->重量级锁

了解下这个数据结构：Markword，待补充



## ReentrantReadWriteLock

ReentrantLock是独占的排它锁，在任一时刻都有一个线程可以持有这个锁。在应对读多写少的场景下，可以使用ReentrantReadWriteLock读写锁。写锁独占，读锁共享：可以多个读操作同时访问，或者一个写操作访问，当时两者不能同时进行。
